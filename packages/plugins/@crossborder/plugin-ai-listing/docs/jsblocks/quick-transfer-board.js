...
const { Card, Table, Tag, Button, Space, Typography, Tooltip, Empty, message } = ctx.libs.antd;
...
function QuickTransferBoard() {
  ...
  const load = useCallback(async (silent) => {
    ...
    try {
      ...
      // 三层信封:HTTP data → 业务 {ok,data} 信封 → {rows}。逐层防御剥壳。
      const payload = (resp && resp.data && resp.data.data) || (resp && resp.data) || resp || {};
      const inner = (payload && payload.data) || payload || {};
      setRows(Array.isArray(inner.rows) ? inner.rows : Array.isArray(payload.rows) ? payload.rows : []);
    } catch (e) {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (state, row) => {
        ...
        return state === 'failed' && (row.errorCode || row.errorMessage) ? (
          <Tooltip title={`${row.errorCode || ''} ${row.errorMessage || ''}`.trim()}>{tag}</Tooltip>
        ) : (
          tag
        );
      },
